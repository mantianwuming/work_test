...
void combination(int* input,int inputLen,int m,int* out,int outLen,int* volume,int reactorCap,int* masses,int criticalMass,int numberOfRadLiquid,int* energies,int energiesMax ) 
{
	...
	if(m==0) // 退出递归，已取到一种情况
	{
		...
		{
			if(energiesSum>energiesMax) // 判断是否比上一个存储的值大
			{
				energiesMax=energiesSum;
...
{	
	...
	int energiesMax = 1; //输出最大能量
	int max;
	max = getMaxEnergies(volume,reactorCap,masses,criticalMass,numberOfRadLiquid,energies);
	printf("最大能量为");
	printf("%d",max);
}